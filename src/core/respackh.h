...
_core_api void *respack_object_imp(const ResPack *pack, const ResId id, FPtr_from_data func_create, FPtr_destroy func_destroy);

_core_api void *respack_aobj_imp(const ArrPt(ResPack) *packs, const ResId id, FPtr_from_data func_create, FPtr_destroy func_destroy, bool_t *is_resid);
...
#define respack_object(pack, id, func_create, func_destroy, type) \
    ((void)FUNC_CHECK_FROM_DATA(func_create, type), \
     ...
     cast(respack_object_imp(pack, id, (FPtr_from_data)func_create, (FPtr_destroy)func_destroy), type))
